...
router.post('/filter', async function (req, res) {
    ...
    var Qlist = req.body
    var model = null
    if (Qlist.name == '' && Qlist.status != '') {
        model = await Model.find({ status: Qlist.status });
    }
    else if (Qlist.name != '' && Qlist.status == '') {
        model = await Model.find({ name: Qlist.name });
    } else {
        model = await Model.find({ name: Qlist.name, status: Qlist.status });
    }
    res.send(model)
})
